-- One vehicle cannot be assigned to two trips on the same calendar date.
-- Existing duplicates are preserved so the owner can correct them explicitly.
CREATE OR REPLACE FUNCTION prevent_vehicle_trip_date_conflict()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
    -- Serialize concurrent writes for the same vehicle and date.
